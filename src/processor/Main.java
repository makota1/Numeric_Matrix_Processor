...
public class Main {
        ...
        if (matrix == null) {
            ...
            for (double[] row : matrix) {
                for (int j = 0; j < row.length; j++) {
                    if (df.format(row[j] + 0).length() > max[j]) {
                        max[j] = df.format(row[j] + 0).length();
                    }
                }
            }
    ...
    private static Double matrixDeterminant(double[][] matrix) {
        if (matrix.length == 2) {
            ...
            for (int j = 0; j < matrix.length; j++) {
                if (j % 2 == 0) {
                    det += matrix[0][j] * matrixDeterminant(matrixMinor(matrix, 0, j));
                } else {
                    det -= matrix[0][j] * matrixDeterminant(matrixMinor(matrix, 0, j));
                }
            }
    ...
    private static double[][] matrixCofactor(double[][] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                if ((i + j) % 2 == 0) {
                    cofactor[i][j] = matrixDeterminant(matrixMinor(matrix, i, j));
                } else {
                    cofactor[i][j] = -1 * (matrixDeterminant(matrixMinor(matrix, i, j)));
                }
            }
        }
        return cofactor;
